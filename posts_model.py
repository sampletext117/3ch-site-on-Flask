class PostModel:
    ...
    def get_all(self, thread_id, user_id=None):
        ...
        if user_id:
            cursor.execute(
                "SELECT * FROM posts WHERE user_id = ? AND thread_id = ?",
                ((str(user_id),
                  thread_id,
                  )))
        else:
    ...
    def delete(self, thread_id, posts_id, user_id):
        cursor = self.connection.cursor()
        cursor.execute(
            '''DELETE FROM posts WHERE id = ? AND thread_id = ? AND user_id = ?''',
            (str(posts_id),
             thread_id,
             user_id),
        )
    ...
    def delete_all(self, thread_id, posts_id):
        cursor = self.connection.cursor()
        cursor.execute(
            '''DELETE FROM posts WHERE id = ? AND thread_id = ?''',
            (str(posts_id),
             thread_id),
        )
        cursor.close()
